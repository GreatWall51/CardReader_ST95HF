...
//	debug("[%d] EXEC drv95HF_SendIRQINPulse\r\n",__LINE__);//pass
...
//	drv95HF_SendReceive(command, u95HFBuffer);
	do{
		...
		drv95HF_SendReceive(command, u95HFBuffer);
//		debug("[%d] drv95HF_SendReceive sak is %x\r\n",__LINE__,u95HFBuffer[0]);
		if (u95HFBuffer[0]==ECHORESPONSE)
		{
//		    debug("[%d] send an ECHO command and checks response SUCCESS!!!!\r\n",__LINE__);
			return MANAGER_SUCCESSCODE;	}
...
void ConfigManager_HWInit (void)
{
	...
	{
//		debug("ConfigManager_PORsequence error!\r\n");
...
//	}
	...
	if ((tagsToFind&TRACK_NFCTYPE2) || (tagsToFind&TRACK_NFCTYPE4A))
	{
		...
		if(scan_ISO14443A_card(uid) == ISO14443A_SUCCESSCODE)
		{
...
//			{				    
//			    debug("%x %x %x %x\r\n",ISO14443A_Card.UID[0],ISO14443A_Card.UID[1],ISO14443A_Card.UID[2],ISO14443A_Card.UID[3]);
//				if (((ISO14443A_Card.SAK&0x60) == 0x00) && tagsToFind&TRACK_NFCTYPE2) /* TT2 */
//				{
//				debug("TRACK_NFCTYPE2\r\n");
////					return TRACK_NFCTYPE2;
//					}
//				else if (((ISO14443A_Card.SAK&0x08) == 0x08) && tagsToFind&TRACK_NFCTYPE4A)/* TT4A *///IC卡SAK:0x08,CPU卡SAK:0x28
//				{
...
//	}
	...
	if (tagsToFind&TRACK_NFCTYPE4B)
	{
	  uint8_t uid[10];
		PCD_FieldOff();
		delay_ms(5);
		if(scan_ISO14443B_card(uid) == ISO14443B_SUCCESSCODE)
		{
		    uint8_t test_res=0x88;
		    debug("[%d]ISO14443B_Card UID is:\r\n",__LINE__);
		    for(test_res=0;test_res<ISO14443B_Card.UIDsize;test_res++)
		    {
		        debug("%x ",uid[test_res]);
		    }
		    debug("\r\n");
		}
...
//			    for(test_res=0;test_res<ISO14443B_Card.UIDsize;test_res++)
//			    {
//			        debug("%x ",ISO14443B_Card.UID[test_res]);
//			    }
//			    debug("\r\n");
