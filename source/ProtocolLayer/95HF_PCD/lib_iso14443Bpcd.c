...
static void ISO14443B_InitStructure ( void )
{
	...
	ISO14443B_Card.UIDsize = 0;
	memset(ISO14443B_Card.UID, 0x00, ISO14443B_MAX_UID_SIZE);
...
static void ISO14443B_CompleteStruture_UID(ISO14443B_CARD * card_b,uint8_t *pDataRead )
{
    card_b->UIDsize = 8;
	memcpy(card_b->UID,&pDataRead[2],card_b->UIDsize);
}
...
#if DEBUG_SFZ_CONFIG
    debug("send ISO14443B_ReqB cmd:\r\n");
	for(test_index=0;test_index<3;test_index++)
	{
	    debug("%x ",ReqB[test_index]);
	}
	debug("\r\n");
#endif
...
#if DEBUG_SFZ_CONFIG
    debug("[%d]send ISO14443B_ReqB ASK:\r\n",__LINE__);
...
#if DEBUG_SFZ_CONFIG
	debug("[%d]send ISO14443B_Anticollision ask:\r\n",__LINE__);
	for(test_index=0;test_index<pDataRead[1]+2;test_index++)
	{
	    debug("%x ",pDataRead[test_index]);
	}
...
#if DEBUG_SFZ_CONFIG
    debug("ISO14443B_Init SUCCESS!!!\r\n");
#endif
	/* WakeUp attempt */
	errchk(ISO14443B_ReqB(pDataRead));
#if DEBUG_SFZ_CONFIG
	debug("[%d]ISO14443B_ReqB success!\r\n",__LINE__);
#endif
	return ISO14443B_SUCCESSCODE;
Error:
	return ISO14443B_ERRORCODE_DEFAULT; 
...
int8_t ISO14443B_Anticollision(void)
{
	...
	nfc_tagtype = TT4B;
#if DEBUG_SFZ_CONFIG
	debug("[%d]ISO14443B_Anticollision success!\r\n",__LINE__);
#endif
	return ISO14443B_SUCCESSCODE;
Error:
	return ISO14443B_ERRORCODE_DEFAULT; 
...
/******************************************************************************
* Name: 	 ISO14443B_Select 
*
* Function Desc: 	 选中卡,获得卡号
* Input Param: 	 
* Output Param: 	 
* Return Value: 	 
* Global: 	 
...
int8_t ISO14443B_Select(void)
{
    ...
    uint8_t *pDataRead = u95HFBuffer;
    int8_t	status;
    uint8_t SelectB[ISO14443B_SeleckB_SIZE]={0x00,0x36,0x00,0x00,0x08};
#if DEBUG_SFZ_CONFIG
    debug("send ISO14443B_Select cmd:\r\n");
	for(test_index=0;test_index<5;test_index++)
	{
	    debug("%x ",SelectB[test_index]);
	}
	debug("\r\n");
#endif
    errchk(PCD_SendRecv(0x05,SelectB,pDataRead));
#if DEBUG_SFZ_CONFIG
    debug("[%d]send ISO14443B_Select ask:\r\n",__LINE__);
	for(test_index=0;test_index<pDataRead[1]+2;test_index++)
	{
	    debug("%x ",pDataRead[test_index]);
	}
	debug("\r\n");
#endif
	ISO14443B_CompleteStruture_UID(&ISO14443B_Card,pDataRead);
#if DEBUG_SFZ_CONFIG
    debug("[%d]ISO14443B_Select success!\r\n",__LINE__);
#endif
	return ISO14443B_SUCCESSCODE;
Error:
	return ISO14443B_ERRORCODE_DEFAULT; 
...
{
    uint8_t result = ISO14443B_ERRORCOD_NOTAG;
    
    if(uid == NULL) return ISO14443B_ERRORCOD_NOTAG;
    
    result = ISO14443B_IsPresent();
    if(result!=ISO14443B_SUCCESSCODE) return ISO14443B_ERRORCOD_REQB;

    result = ISO14443B_Anticollision();
    if(result!=ISO14443B_SUCCESSCODE) return ISO14443B_ERRORCOD_ANTICOLLB;

    result = ISO14443B_Select();
    if(result!=ISO14443B_SUCCESSCODE) return ISO14443B_ERRORCOD_SELECTB;

    memcpy(uid,ISO14443B_Card.UID,ISO14443B_Card.UIDsize);
    return ISO14443B_SUCCESSCODE;
}
