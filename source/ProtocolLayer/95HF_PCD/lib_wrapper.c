...
//			memcpy(pData,&CardNDEFfileT4B[Offset],DataSize);
		...
		case TT5:
          ...
          if(pTT5Tag[2] != 0)
          {
            ...
            if (pTT5Tag[5] == 0xFF)
            /* 2 bytes length */
              pData[i-Offset]=pTT5Tag[6+i];
            else
             /* 1 bytes length */
             pData[i-Offset]=pTT5Tag[4+i];
          } else {
            /* 8 bytes CCfile */
             if (pTT5Tag[9] == 0xFF)
            /* 2 bytes length */
              pData[i-Offset]=pTT5Tag[10+i];
            else
             /* 1 bytes length */
             pData[i-Offset]=pTT5Tag[8+i];           
          }
				}
			}
			break;
...
//					updateFlash = true;
		...
		case TT5:
			...
			if (DataSize < 255)
			{
				pTT5Tag[index++] = DataSize;
			}
			else
			{
				pTT5Tag[index++] = 0xFF;
				pTT5Tag[index++] = (DataSize&0x0000FF00)>>8;
				pTT5Tag[index++] = DataSize&0x000000FF;
			}
			memcpy(&(pTT5Tag[index]),pData+2, DataSize);
			/* Write the terminating TLV [0xFE] */
			pTT5Tag[index+DataSize] = 0xFE;
			/* Update the phisical tag if needed */
			#if defined(ST95HF) || defined(CR95HF)
			if (devicemode == PCD)
