...
/**
 *	@brief  this functions turns the field off: 2 2 0 0
 *  @param  none
 *  @retval PCD_SUCCESSCODE : the function is succesful 
 */
 //send:   2 2 0 0
...
void PCD_FieldOff( void )
{
	...
	PCD_ProtocolSelect(0x02, PCD_PROTOCOL_FIELDOFF, &ParametersByte,u95HFBuffer);
}
...
void PCD_FieldOn ( void )
{
	...
	PCD_ProtocolSelect(0x02, PCD_PROTOCOL_ISO15693, &ParametersByte,u95HFBuffer);
...
* Name: 	 PCD_Send 
*
* Function Desc: 	 Ö»·¢ËÍÊý¾Ý,²»µÈ´ýÓ¦´ð
* Input Param: 	 
* Output Param: 	 
* Return Value: 	 
* Global: 	 
...
* Author: 	 liuwq
* ----------------------------------------------------------------------------
* Log: 	 2017/06/07, Create this function by liuwq
******************************************************************************/
int8_t PCD_Send(uc8 Length,uc8 *Parameters,uint8_t *pResponse)
{
	uint8_t DataToSend[SENDRECV_BUFFER_SIZE];
	...
	*(pResponse+1) = 0x00;
	...
	DataToSend[PCD_COMMAND_OFFSET ] = SEND;
	DataToSend[PCD_LENGTH_OFFSET  ]	= Length;
...
/******************************************************************************
* Name: 	 Modulation_Control 
*
* Function Desc: 	 ÐÞ¸Äµ÷ÖÆÉî¶È,Ã¿´ÎÇÐ»»Ð­Òé¶¼»áÑ¡Ôñ,ÔÚÕâÀïÉèÖÃ²»ÉúÐ§
* Input Param: 	 Modulation_reg:¼Ä´æÆ÷µØÖ·,ACC_A = 0x04,ACC_B = 0x01
                 val:ÐÂµÄÖµ
* Output Param: 	 
* Return Value: 	 
* Global: 	 
...
* Author: 	 liuwq
* ----------------------------------------------------------------------------
* Log: 	 2017/05/16, Create this function by liuwq
******************************************************************************/
int8_t Modify_Modulation(uint8_t Modulation_reg,uint8_t val)
{
//    u8     ProtocolSelectParameters [4]  = {0x51, 0x13, 0x01,0x0D};
		u8     ProtocolSelectParameters [6]  = {0x0, 0x04, 0x0,0x12,0x03,0x03};
    uint8_t *pDataRead = u95HFBuffer;
    uint8_t Write_Modulation[2]={0};
    Write_Modulation[0] = Modulation_reg;
    Write_Modulation[1] = val;
    //1.Ð­ÒéÑ¡Ôñ
    PCD_ProtocolSelect(0x07,PCD_PROTOCOL_ISO14443A,ProtocolSelectParameters,pDataRead);
//    debug("[%d] protocol select result = %x %x\r\n",__LINE__,pDataRead[0],pDataRead[1]);

    //2.¶Á
    PCD_WriteRegister(0x03,0x68,0x00,&Write_Modulation[0],pDataRead);
//    debug("[%d] write modulation addr result = %x %x\r\n",__LINE__,pDataRead[0],pDataRead[1]);

	PCD_ReadRegister(0x03,0x69,0x01,0x01,pDataRead);
//	debug("[%d] read modulation result = %x %x %x\r\n",__LINE__,pDataRead[0],pDataRead[1],pDataRead[2]);

	//3.ÐÞ¸Ä
    PCD_WriteRegister(0x04,0x68,0x01,Write_Modulation,pDataRead);
//    debug("[%d] modify modulation result = %x %x\r\n",__LINE__,pDataRead[0],pDataRead[1]);

    //4.¶Á³öÐÞ¸ÄºóÊý¾Ý
    PCD_WriteRegister(0x03,0x68,0x01,&Write_Modulation[0],pDataRead);
//    debug("[%d] write modulation addr result = %x %x\r\n",__LINE__,pDataRead[0],pDataRead[1]);
