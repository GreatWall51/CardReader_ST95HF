...
#define PCD_TYPEV_ARConfigB	0xdf//0xD1
...
/******************************************************************************
* Name: 	 ISO15693_CreateRequestFlag 
*
* Desc: 	 生成请求码(一字节，8bit分别如下输入参数)
* Param(in): 	 bit1:SubCarrierFlag 副载波标志，0->VICC应使用单个副载波频率,1->VICC应使用两个副载波频率
                 bit2:DataRateFlag   数据速率标志 0->低速,1->高速
                 bit3:InventoryFlag  目录标志,决定bit5-bit8的含义
                 bit4:ProtExtFlag    协议拓展标志,0->无协议拓展,1->协议格式已拓展
(bit5-bit8由InventoryFlag确定)
当InventoryFlag = 0时:
                 bit5:Select         选择标志,0->请求指令由任何VICC执行,1->请求指令由指定的VICC执行
                 bit6:Addr           寻址标志,0->没寻址，所有VICC都可响应请求指令,1->请求指令由特定的VICC响应
                 bit7:OptionFlag     选择权标志,含义由命令描述定义，如果没有被命令定义，它应设置为 0
                 bit8:RFUFlag        预留
当InventoryFlag = 1时:
                 bit5:AFIFlag        应用族标志,0->AFI域没有出现,1->AFI域有出现
                 bit6:NbSlotFlag     时隙标志就是序号(参数),0->16 slots请求参数可以是0到60之间的任何值,1->1slots请求参数可以是0到64之间的任何值
                 bit7:OptionFlag     选择权标志,含义由命令描述定义，如果没有被命令定义，它应设置为 0
                 bit8:RFUFlag        预留
* Param(out): 	 无
* Return: 	 Flags byte 请求码
* Global: 	 
* Note: 	 this function return a Byte, which is concatenation of iventory flags 
             VCD:读卡器， VICC:符合ISO15693协议的卡片
* Author: 	 liuwq
*------------------------------------------------------------------------------
* Log: 	 2017/09/12, Create this function by liuwq
*******************************************************************************/
static int8_t ISO15693_CreateRequestFlag (uint8_t SubCarrierFlag,uint8_t DataRateFlag,uint8_t InventoryFlag,uint8_t ProtExtFlag,
                                                            uint8_t SelectOrAFIFlag,uint8_t AddrOrNbSlotFlag,uint8_t OptionFlag,uint8_t RFUFlag)
{
...
/******************************************************************************
* Name: 	 ISO15693_Inventory 
*
* Desc: 	 this function send an inventory(anti-collision) command to a contacless tag. 
* Param(in): 	Flags:Request flags请求标志
                AFI:AFI byte (optional)应用族标志
                MaskLength:Number of bits of mask value(注意是比特长度，不是字节长度)
                MaskValue:mask value which is compare to Contacless tag UID
                pResponse:pointer on PCD  response
* Param(out): 	 
* Return: 	ISO15693_SUCCESSCODE	: 	PCD  returns a succesful code
            ISO15693_ERRORCODE_PARAMETERLENGTH	: 	MaskLength value is erroneous
            ISO15693_ERRORCODE_DEFAULT	: 	 PCD  returns an error code
* Global: 	 
* Note: 	 
* Author: 	 liuwq
*------------------------------------------------------------------------------
...
static int8_t ISO15693_Inventory(const uint8_t Flags,const uint8_t AFI,const uint8_t MaskLength,const uint8_t *MaskValue,uint8_t *pResponse)
{
	...
	*(pResponse+1) = 0x00;
    ...
    //check whether InventoryFlag is set or not
	...
	//check whether AFIFlag is set or not
	...
	if(MaskLength !=0)
	{
		...
		//假如MaskLength长度不是8（bits）的倍数，MaskLength值的最高有效位（ MSB）将补0，使得MaskLength值是整数个字节。
	   	if(MaskLength == 64)
			NbMaskBytes = 8;
		else 
...
/******************************************************************************
* Name: 	 ISO15693_ReadSingleTagData 
*
* Desc: 	 Read Single Block in the TAG
* Param(in): 	 Tag_Density:TAG is HIGH or LOW density(卡容量)
                 NbBlock_To_Read:Number of block to read in the TAG
                 FirstBlock_To_Read:First block to read in the TAG
* Param(out): 	 Data_To_Read:the data read in the TAG
* Return: 	 
* Global: 	 
* Note: 	 
* Author: 	 liuwq
*------------------------------------------------------------------------------
* Log: 	 2017/09/14, Create this function by liuwq
*******************************************************************************/
static uint8_t ISO15693_ReadSingleTagData(uint8_t Tag_Density, uint8_t *Data_To_Read, uint16_t NbBlock_To_Read, uint16_t FirstBlock_To_Read)
{
...
{ 
    ...
    uint8_t	TagReply[ISO15693_NBBYTE_UID+256];
    uint8_t Tag_error_check;
	

	/* select 15693 protocol */
	errchk(ISO15693_Init());
    /*Create Request Flag,0x26*/
	FlagsByteData = ISO15693_CreateRequestFlag(ISO15693_REQFLAG_SINGLESUBCARRIER,
                                                    ISO15693_REQFLAG_HIGHDATARATE,
                                                    ...
                                                    ISO15693_REQFLAG_RFUNOTSET);
	/*anti-collision*/
	errchk(ISO15693_Inventory(FlagsByteData,
						 	0x00, 
						 	0x00,
							0x00,
							TagReply ));
    ...
    if(TagReply[1] > 0x1d) return ISO15693_ERRORCODE_DEFAULT;
	if (status == ISO15693_SUCCESSCODE)
		memcpy(UIDout,&(TagReply[ISO15693_OFFSET_UID]),ISO15693_NBBYTE_UID);
//    debug("[%d] ISO15693_GetUID success!\r\n",__LINE__);
//    debug("[%d] devicemode = PCD!\r\n",__LINE__);
//    debug("[%d] nfc_tagtype = TT5!\r\n",__LINE__);
	devicemode = PCD;
	nfc_tagtype = TT5;
	return ISO15693_SUCCESSCODE; 
Error:
...
{
    uint8_t result = ISO15693_ERRORCODE_DEFAULT;
    if(uid == NULL) return result;
    
    PCD_FieldOff();
    result = ISO15693_GetUID(uid);
    return result;
}
